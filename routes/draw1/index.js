...
const fs = require("fs");
const qs = require("qs");
const kakao = require("../../kakao").kakao1;
const drawID = "draw1";
...
router.get("/subscribe/callback", async (req, res) => {
  ...
  if (user.data.kakao_account.email == "kdhak2@gmail.com") {
    if (token.data.scope.includes("talk_message"))
      fs.writeFileSync(kakao.tokenPath, JSON.stringify(token.data));
  }
  ...
  const found = users[drawID].some(
    (el) =>
      el.kakao_account.profile.nickname ===
